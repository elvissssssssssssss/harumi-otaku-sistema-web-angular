...
export class EnvioVoletaService {
 ...
 private handleError(error: HttpErrorResponse): Observable<never> {
  ...
  if (error.error instanceof ErrorEvent) {
    ...
    switch (error.status) {
      case 400: errorMessage = 'Solicitud incorrecta. Verifique los datos enviados.'; break;
      ...
      default:  errorMessage = `Error del servidor: ${error.status} - ${error.message}`;
    }
  }

  // Adjuntar mensaje “amigable” sin perder el status
...
return this.http.get<SeguimientoEnvio[]>(`${this.apiUrl}/mis-seguimientos`, { headers }).pipe(
  retry(2),
